        ...
        if (sessionState.authState === AuthState.UNAUTHENTICATED) {
            ...
            if (userRef.current) {
                userRef.current.globalSignOut({onSuccess: (msg: string) => {
                    ...
                    window.location.href = `https://stagingpagopeople.auth.us-west-2.amazoncognito.com/login?client_id=${userPoolClientId}&response_type=code&scope=aws.cognito.signin.user.admin+email+openid+phone+profile&redirect_uri=${getRedirectUri()}`;
                },
                onFailure: (err: Error) => {
